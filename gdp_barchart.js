...
d3.csv("/dataset/GDP-Current.csv").then((wide_data) => {
  ...
  newData = newData.get("Saudi Arabia").get("GDP (current US$)")
  newData = d3.filter(newData, (d) => (2000 <= d.Year && d.Year <= 2022))
...
function makeVis(data) {
  ...
  // create a tooltip
  const tooltip = d3.select("#container2")
    .append("div")
    .attr("class", "tooltip")
  ...
  var dropdownChange = function () {
    ...
    newData = newData.get(newCountry).get("GDP (current US$)")
    newData = d3.filter(newData, (d) => (2000 <= d.Year && d.Year <= 2022))
  ...
  var dropdown = d3.select("#container2")
    .insert("select", "svg")
